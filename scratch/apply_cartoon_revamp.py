# -*- coding: utf-8 -*-
"""
Script d'application des 4 nouveaux cartoons et de la nouvelle fenêtre de message dans ghost_script.py.
Sauvegarde automatique et vérification par py_compile.
"""
...
BACKUP_PATH = "ghost_script.py.bak_cartoons"

print(f"1. Sauvegarde de {SCRIPT_PATH} vers {BACKUP_PATH}...")
...
# ==============================================================================
# REMPLACEMENT 1 : show_troll_window_on_client
# ==============================================================================
print("2. Remplacement de show_troll_window_on_client...")

target_start_1 = "def show_troll_window_on_client(text):"
idx_start_1 = content.find(target_start_1)
if idx_start_1 == -1:
    print("ERREUR: Impossible de trouver target_start_1")
    sys.exit(1)

target_end_1 = "# ==============================================================================\n# AUDIO LOOPBACK AND CAPTURE SUPPORT"
idx_end_1 = content.find(target_end_1, idx_start_1)
if idx_end_1 == -1:
    print("ERREUR: Impossible de trouver target_end_1")
    sys.exit(1)

new_troll_window = '''def show_troll_window_on_client(text):
    """
    ...
    def _create_gui():
        ...
        try:
            ...
            try:
                ...
                win.geometry(f"{w}x{h}+300+180")
                
            win.resizable(False, False)
            win.attributes("-topmost", True)
            win.lift()
            
            # Récupérer le HWND et l'ajouter aux fenêtres autorisées
            win.update()
            ...
            def _render():
                ...
                # Corde élastique / Bungee depuis le haut de l'écran jusqu'à la casquette
                ...
                # Nuages de poussière cartoon à l'atterrissage
                for dc in anim_data["dust_clouds"]:
                    dr = dc["r"]
                    canvas.create_oval(dc["x"] - dr, dc["y"] - dr*0.6, dc["x"] + dr, dc["y"] + dr*0.6, fill="#e2e8f0", outline="#cbd5e1", width=1)
                    
                # Grand Panneau de Bois et Parchemin Chaleureux (Zéro néon !)
                board_y = 150 + cur_oy
                bw, bh = 560, 310
                bx1 = cx - bw // 2
                by1 = board_y
                ...
                canvas.create_text(cx, rub_y, text="📬 LIVRAISON EXPRESS D'UN MESSAGE 📬", font=("Impact", 13, "bold"), fill="#ffffff")
                
                display_msg = text if text else "Aucun message reçu."
                if not display_msg.startswith("AHMED:") and not display_msg.startswith("MESSAGE:"):
                    display_msg = f"MESSAGE DU VIEWER :\\n« {display_msg} »"
                ...
                # Grand Personnage Cartoon Perché au-dessus du Panneau
                char_x = cx
                char_y = board_y - 20
                
                canvas.create_line(char_x - 35, char_y + 10, bx1 + 35, by1 + 10, fill="#1e3a8a", width=12, capstyle=tk.ROUND)
                canvas.create_oval(bx1 + 25, by1, bx1 + 45, by1 + 20, fill="#ffffff", outline="#0f172a", width=2)
                
                canvas.create_line(char_x + 35, char_y + 10, bx2 - 35, by1 + 10, fill="#1e3a8a", width=12, capstyle=tk.ROUND)
                canvas.create_oval(bx2 - 45, by1, bx2 - 25, by1 + 20, fill="#ffffff", outline="#0f172a", width=2)
...
content = content[:idx_start_1] + new_troll_window + content[idx_end_1:]

# ==============================================================================
# REMPLACEMENT 2 : Clavier farceur dans keyboard_hook_callback
# ==============================================================================
print("3. Mise à jour de l'interception de touche dans keyboard_hook_callback...")

old_hook_code = '''            # Clavier farceur (Prank keyboard keys) - Intercepter et remplacer les lettres
            if prank_keys_active and not (flags & 0x00000010):
                if 0x41 <= vk <= 0x5A:  # Touches A-Z
                    if wParam in (256, 260):  # keydown
                        vowel = random.choice([0x41, 0x45, 0x49, 0x4F, 0x55]) # A, E, I, O, U
                        ...
                        if keys_overlay_instance is not None:
                            try:
                                keys_overlay_instance.trigger_hit(chr(vowel))
                            except Exception:
                                pass
                    return 1  # Bloquer l'événement d'origine'''

new_hook_code = '''            # Clavier farceur (Prank keyboard keys) - Intercepter et remplacer les lettres avec animation hyper-réaliste
            if prank_keys_active and not (flags & 0x00000010):
                if 0x41 <= vk <= 0x5A:  # Touches A-Z
                    if wParam in (256, 260):  # keydown
                        user_char = chr(vk)
...
idx_hook = content.find(old_hook_code)
if idx_hook == -1:
    print("ERREUR: Impossible de trouver old_hook_code dans keyboard_hook_callback")
    sys.exit(1)

content = content[:idx_hook] + new_hook_code + content[idx_hook + len(old_hook_code):]

# ==============================================================================
# REMPLACEMENT 3 : Les 3 classes d'overlays cartoon
# ==============================================================================
print("4. Remplacement des 3 classes cartoon (Peintre, Clavier, Fantôme)...")

target_start_3 = "class CartoonPainterOverlay:"
idx_start_3 = content.find(target_start_3)
if idx_start_3 == -1:
    print("ERREUR: Impossible de trouver target_start_3")
    sys.exit(1)

target_end_3 = "class CartoonMegaphoneOverlay:"
idx_end_3 = content.find(target_end_3, idx_start_3)
if idx_end_3 == -1:
    print("ERREUR: Impossible de trouver target_end_3")
    sys.exit(1)

three_new_cartoons = '''class CartoonPainterOverlay:
    """
    Peintre cartoon français hyper-réaliste :
    ...
    def _draw_painter(self):
        ...
        # Bras droit & Pinceau selon l'animation
...
content = content[:idx_start_3] + three_new_cartoons + content[idx_end_3:]

with open(SCRIPT_PATH, "w", encoding="utf-8") as f:
    f.write(content)

print(f"5. Compilation et validation de {SCRIPT_PATH}...")
py_compile.compile(SCRIPT_PATH, doraise=True)
print("SUCCESS: Toutes les modifications ont été appliquées et compilées avec succès !")
